...
class MPTModel(ComposerMPTCausalLM):
    ...
    def __init__(self, config, tokenizer):
        ...
        self.pertoken_loss_fct = CrossEntropyLoss(
    ...
    def forward(
        ...
        domain_ids: Optional[torch.LongTensor] = None,
        return_pertoken_losses: Optional[bool] = False,
        tokens=None,  # So that the trainer doesn't rm before collator,
        sequence_id=None,
    ) -> Union[Tuple, CausalLMOutputWithDomainIDs]:
        if not return_pertoken_losses:
            ...
            if not return_dict:
                output = (lm_logits, None, None, None, domain_ids,
...
def load_mpt_model(model_name, tokenizer, ckpt_path=None):
    init_context = contextlib.nullcontext()
    with init_context:
        if model_name == "mpt-150M":
            model_cfg = om.create({
                "name": "mpt_causal_lm",
                "d_model": 768,
                "n_heads": 12,
