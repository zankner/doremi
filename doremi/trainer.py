...
from transformers.trainer_utils import (has_length, denumpify_detensorize,
                                        EvalLoopOutput,
...
class LinearWarmupExponentialLR(LRScheduler):
    ...
    def __init__(self,
        ...
        self.gammas = [
            np.exp(
                np.log(1e-10 / (base_lr - self.lr_end)) /
    ...
    def _get_closed_form_lr(self):
        if self.last_epoch < self.num_warmup_steps:
            return [
                self.lr_start + (base_lr - self.lr_start) * self.last_epoch /
                self.num_warmup_steps for base_lr in self.base_lrs
            ]
        else:
            return [
                self.lr_end + (base_lr - self.lr_end) *
                gamma**(self.last_epoch - self.num_warmup_steps)
                for gamma, base_lr in zip(self.base_lrs, self.gammas)
            ]
...
class LinearWarmupCosineLR(LRScheduler):
    ...
    def _get_closed_form_lr(self):
        if self.last_epoch < self.num_warmup_steps:
            ...
            return [
                self.lr_end + (base_lr - self.lr_end) * (1 + math.cos(
                    math.pi * (self.last_epoch - self.num_warmup_steps) /
                    (self.num_training_steps - self.num_warmup_steps))) / 2
                for base_lr in self.base_lrs
            ]
...
class DoReMiTrainer(Trainer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.train_domain_weights_dict = self.domain_config[
            'train_domain_weights']
        self.eval_domain_weights_dict = self.domain_config[
            'eval_domain_weights']

        self.domain_list = PILE_NAMES_ORDERED
        self.sampling_weights = torch.tensor([
            self.train_domain_weights_dict[domain]
    ...
    def write_weights(self, weights):
        ...
        self.model.avg_domain_weights[:] = (
            self.model.avg_domain_weights * (self.model.update_counter - 1) +
            weights) / self.model.update_counter
    ...
    def compute_loss(self,
        ...
        if return_outputs:
            ...
            return (loss, outputs['pertoken_loss'],
                    outputs['reference_pertoken_loss'], outputs['token_mask'])
        else:
            return loss
    ...
    def update_domain_weights(self, scores, scores_mask, domain_ids):
        ...
        if self.args.doremi_optimizer == 'doremiv1':
            ...
            log_new_train_domain_weights = torch.log(
                train_domain_weights
            ) + self.args.reweight_eta * self.model.perdomain_scores
            new_train_domain_weights = nn.functional.softmax(
                log_new_train_domain_weights, dim=0)
            train_domain_weights = (
                1 - self.args.reweight_eps
            ) * new_train_domain_weights + self.args.reweight_eps / len(
                new_train_domain_weights)
            self.write_weights(train_domain_weights)
        else:
            raise ValueError(
                f"DoReMi optimizer {self.args.doremi_optimizer} not supported")

        wandb_log_dict = {}
        for domain_idx in range(len(new_train_domain_weights)):
            domain_name = self.domain_list[domain_idx]
            wandb_log_dict[
                f'avg_domain_weights/{domain_name}'] = self.model.avg_domain_weights[
                    domain_idx].item()
            wandb_log_dict[
                f'train_domain_weights/{domain_name}'] = self.model.train_domain_weights[
                    domain_idx].item()
            wandb_log_dict[
                f'perdomain_scores/{domain_name}'] = self.model.perdomain_scores[
                    domain_idx].item()
    ...
    def training_step(self, model, inputs):
        ...
        if self.args.reweight_domains:
            with self.compute_loss_context_manager():
                loss, pertoken_loss, reference_pertoken_loss, token_mask = self.compute_loss(
            ...
            if self.is_world_process_zero():
                with torch.no_grad():
                    ...
                    gathered_domain_id = [
                        torch.zeros_like(inputs['domain_ids'])
                        for _ in range(self.args.world_size)
                    ]
                    dist.gather(inputs['domain_ids'],
                                gathered_domain_id,
                                dst=0)
                    gathered_domain_id = torch.cat(gathered_domain_id, dim=0)

                    self.pertoken_scores.append(
                        gathered_excess_losses.detach())
                    self.token_masks.append(gathered_token_mask.detach())
                    self.domain_ids.append(gathered_domain_id.detach())

                    if len(self.pertoken_scores
                           ) == self.args.gradient_accumulation_steps:
                        pertoken_scores = torch.cat(self.pertoken_scores,
                                                    dim=0)
                        token_masks = torch.cat(self.token_masks, dim=0).bool()
                        domain_ids = torch.cat(self.domain_ids, dim=0)

                        # update domain weights
                        self.update_domain_weights(pertoken_scores,
                                                   token_masks, domain_ids)
            ...
            if self.args.doremi_optimizer == 'doremiv1':
                ...
                train_domain_weights = self.read_weights().to(
                    pertoken_loss.device)
                # if doing non-uniform sampling, normalize by inverse sampling weight
                train_domain_weights = train_domain_weights / self.sampling_weights.to(
                    train_domain_weights.device)
                train_domain_weights = train_domain_weights / train_domain_weights.sum(
                )
                curr_domain_weights = train_domain_weights[
                    inputs['domain_ids']].unsqueeze(-1).expand_as(
                        pertoken_loss).detach()
                ...
                raise ValueError(
                    f"doremi_optimizer {self.args.doremi_optimizer} is not supported"
                )
        else:
            with self.compute_loss_context_manager():
    ...
    def load_checkpoint(self, resume_from_checkpoint=None):
        ...
        ) and self.args.deepspeed is None:
    ...
    def get_all_checkpoints(self):
        ...
        checkpoints = [
            os.path.join(folder, path) for path in content
    ...
    def evaluation_loop(
        ...
        for step, inputs in tqdm(enumerate(dataloader)):
            ...
            for domain_idx, domain_name in enumerate(self.domain_list):
                ...
                examplecounts[domain_idx] = examplecounts[
                    domain_idx] + domain_mask.sum()

                if domain_mask.sum() > 0:
                    domain_labels = labels[domain_mask]
                    domain_preds = logits[domain_mask]
                    domain_labels = domain_labels[:, 1:].contiguous().view(-1)
                    domain_preds = domain_preds[:, :-1, :].contiguous().view(
                        -1, domain_preds.size(-1))
                    losses[domain_idx] = losses[domain_idx] + loss_fn(
                        domain_preds, domain_labels)
                    tokencounts[domain_idx] = tokencounts[domain_idx] + (
                        domain_labels != -100).sum()
        ...
        metrics["worst_case_log_perplexity"] = np.amax(
            list(per_domain_losses.values()))
        ...
        return EvalLoopOutput(predictions=None,
                              ...
                              metrics=metrics,
                              num_samples=sum(
                                  list(per_domain_examplecounts.values())))
